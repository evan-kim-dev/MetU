...
export function AirportSearchField({
  ...
  const staticResults = useMemo(
    () => searchAirportPlaces(open ? query : value, 14),
    [open, query, value]
  );
  ...
  useEffect(() => {
    ...
    const timer = window.setTimeout(() => {
      void searchAirportsFromApi(activeQuery, 14).then((places) => {
  ...
  const results = useMemo(
    () => mergeAirportPlaces(staticResults, apiResults, 14),
    ...
    <div ref={rootRef} className="relative flex flex-col gap-1">
      ...
      {open && (
        <ul
          ...
          role="listbox"
          className={[
            "absolute left-0 right-0 z-40 max-h-60 overflow-y-auto border border-line-soft bg-surface-white py-1 shadow-soft",
            isCompact
        ...
        >
          {results.length === 0 ? (
            <li className="px-3 py-2.5 text-sm text-ink-caption">검색 결과가 없어요</li>
          ) : (
                  ...
                  >
                    <span
                      className={[
                        ...
                        isAirport ? "bg-surface-soft text-ink-caption" : "bg-brand/10 text-brand",
                      ].join(" ")}
                    >
                      {isAirport ? (
                        <Plane className={isCompact ? "h-3 w-3" : "h-4 w-4"} strokeWidth={2.2} />
                      ) : (
                        <MapPin className={isCompact ? "h-3 w-3" : "h-4 w-4"} strokeWidth={2.2} />
                      )}
                    </span>
